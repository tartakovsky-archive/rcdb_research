...
def higuchi_fd(series: np.ndarray, window: int, kmax: int = 2) -> np.ndarray:
    """
    ...
    window : int
        Rolling window size
    kmax : int, optional
        Maximum delay/offset (in number of samples). Default is 2

    Returns
    -------
    np.ndarray
        Higuchi Fractal Dimension series
    """
...
def katz_fd(series: np.ndarray, window: int) -> np.ndarray:
    """
    Katz Fractal Dimension
...
def petrosian_fd(series: np.ndarray, window: int) -> np.ndarray:
    """
    Petrosian fractal dimension
    ...
    -------
    np.ndarray
        Petrosian fractal dimension series
    """
    return rolling_apply(
        _petrosian_fd,
        window,
