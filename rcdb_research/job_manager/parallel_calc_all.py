...
from .utils import np_to_file, np_from_file, json_from_file, kwargs_to_str, \
    json_to_folder, chunks, FnSerializer, AttrDict, generate_constraints_function, \
    np_to_file_custom_bytes, np_from_file_custom_bytes
from .transformations import TransformObj, Transforms, TransformsMixin, TransformDelayed
...
class FnTask:
    ...
    def __init__(self, task_meta: Dict, short_feature_names: bool = False):
        self.short_feature_names = short_feature_names
    ...
    def _get_short_name(self):
        ...
        def format_params(argspec, inpnames, params):
            lst = []
            for arg in argspec.args:
                if arg in inpnames:
                    # lst.append(arg if inpnames[arg] == arg else '{}={}'.format(arg, inpnames[arg]))
                    lst.append(str(inpnames[arg]))
                elif arg in params:
                    lst.append('{}={}'.format(arg, params[arg]))
                else:
                    # raise ValueError('fun param not provided: {}'.format(arg))
                    pass
            return ', '.join(lst)
        ...
        result = '{}.{}({})'.format(
            self.prefix,
            fn_name,
            format_params(inspect.getfullargspec(self.fn), self.input_names, self.params)
        )

        if self.transform_post:
            result += '.'.join([''] + [t.get_name() for t in self.transform_post])

        return result

    def get_name(self):
        if self.short_feature_names:
...
class JobTaskWrapper:
    def __init__(
            ...
            short_feature_names: bool = False
    ):
        self.short_feature_names = short_feature_names
        self.data_cache = {}
    ...
    def process_tasks(self, tasks):
        ...
        for task_meta in tasks:
            t = FnTask(task_meta, short_feature_names=self.short_feature_names)
    ...
    def __concurrent_jobs_feature_output_callback(job_meta, task: FnTask, fn_output: np.ndarray):
        ...
        # to bytes
        np_to_file_custom_bytes(task_serialized, feature_output_path, fn_output)
...
class JobHandler:
    ...
    def run_job(self):
        short_feature_names = self.job_meta.get('short_feature_names', False)
        jw = JobTaskWrapper(self.job_meta, short_feature_names=short_feature_names)
        results = []
        task_counter = 0
...
class JobOutput:
    ...
    def __load_output_folder_data(output_folder):
        ...
        for fname in files:
            ...
            yield np_from_file_custom_bytes(os.path.join(output_folder, fname))
...
class ConfigEntry:
    ...
    :param dm: data mapping
    :param pg: parameter grid
    :param tr: post-transforms
    :param cn: condition
    :param alias: alias that will replace the function name
    """
    fn: Callable[..., np.ndarray]
    dm: Dict[str, List[Column]]
    pg: Dict[str, List[Any]] = None
    tr: Optional[List[TransformObj]] = None
    cn: Optional[str] = None
    alias: Optional[str] = None


class JobManager:
    def __init__(self, data: pd.DataFrame, config: Dict[str, List[ConfigEntry]] = None, fn_tasks: List = None,
        ...
        self.job_meta = self.create_job(
            ...
            fn_tasks=copy.deepcopy(fn_tasks)
        )
        self.job_meta['short_feature_names'] = short_feature_names
    ...
    def run_job(self):
        if self.job_meta['n_jobs'] == 1:
            ...
            root_module_name = os.path.basename(
                os.path.dirname(
                    os.path.dirname(__file__)
                )
            )

            command = f"{python_path} -c 'from {root_module_name}.job_manager.parallel_calc_all import" \
    ...
    @staticmethod
    def generate_fn_list_from_config(config: Union[Dict, ConfigEntry], add_feature_ids=False) -> List:
        fn_parallel_list = []

        config_copy = copy.deepcopy(config)

        for prefix, fn_settings_list in config_copy.items():
            for fn_settings in fn_settings_list:
                if isinstance(fn_settings, dict):
                    fn_settings: ConfigEntry = ConfigEntry(**fn_settings)
                ...
                for kwargs in kwargs_list:
                    ...
                    if fn_settings.alias is not None:
        ...
        return fn_parallel_list

    @staticmethod
    def generate_inputs_set(data: pd.DataFrame, fn_parallel_list: List[Dict]) -> set:
        inputs_set = set()
        for task in fn_parallel_list:
            for kwargs in filter(lambda x: isinstance(x, dict) and 'name' in x, task['kwargs'].values()):
                col_name = kwargs['name']
                if col_name in inputs_set:
                    continue

                series = data[col_name]

                if not (str(series.values.dtype).startswith("int") or  # noqa
                        str(series.values.dtype).startswith("float")):  # noqa
                    raise ValueError(f"Unsupported input ({col_name}) type or array "
                                     f"contains unsupported values (None, strings, etc)")

                if not np.isfinite(series).all():
                    raise ValueError(f"{col_name} series contains non-finite values, "
                                     f"e.g. np.inf or np.nan (np.isfinite)")

                inputs_set.add(col_name)

        return inputs_set

    def create_job(
            self, data: pd.DataFrame, config: Dict = None, fn_tasks: List = None,
            n_jobs=1, temp_folder=None, batch_size=1000,
        ...
        if fn_tasks:
            fn_parallel_list = fn_tasks
        else:
            fn_parallel_list = self.generate_fn_list_from_config(config, add_feature_ids)

        inputs_set = self.generate_inputs_set(data, fn_parallel_list)
...
def compute_config(bars, config, batch_size=100, n_jobs=1, tmp='/tmp/jobmanager'):
    import os.path
    os.makedirs(tmp, exist_ok=True)

    if n_jobs == 1:
        jm = JobManager(bars, config=config)
    else:
        jm = JobManager(bars, config=config, n_jobs=n_jobs, batch_size=batch_size, temp_folder=tmp)

    X = jm.run_job().get_pandas()
