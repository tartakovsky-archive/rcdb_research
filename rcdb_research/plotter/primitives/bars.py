...
def bars_colors(pos: str = 'deepskyblue', neg: str = 'tomato') -> dict: return locals()
...
def bars(y: np.array, x: np.array = None, threshold: float = 0,
    ...
    if np.any(y >= threshold):
        ...
        axis.bar(x, height=y_pos, bottom=threshold, color=colors['pos'], label=legend['pos'])

    if np.any(y < threshold):
        y_neg = np.where(y < threshold, y - threshold, np.nan)
        axis.bar(x, height=y_neg, bottom=threshold, color=colors['neg'], label=legend['neg'])
