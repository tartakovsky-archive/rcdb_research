...
def trading_report(trades: Trades, show_dates: bool = False):
    fig_kwargs = style.fig_kwargs(figsize=(16, 9))
    ax_kwargs = style.ax_kwargs(
        xformatter=ticker.FormatStrFormatter('%.0f'),
        yformatter=ticker.PercentFormatter(xmax=1, decimals=0),
    ),
    line_kwargs = style.line_kwargs(linewidth=1)
    ...
    primitives.line_pn(trades.metrics.cum_return(),
                       title='Cumulative return over bars',
                       ylabel='Gain',
                       fill=True,
                       ax_kwargs={**ax_kwargs, 'ylocator': ticker.MaxNLocator(12)},
                       pos_line_kwargs=line_kwargs,
                       neg_line_kwargs=line_kwargs,
                       ax=ax0)
    ...
    primitives.line_pn(trades.metrics.returns(),
                       ylabel='Returns',
                       ax_kwargs={**ax_kwargs, 'ylocator': ticker.MaxNLocator(5)},
                       pos_line_kwargs=line_kwargs,
                       neg_line_kwargs=line_kwargs,
